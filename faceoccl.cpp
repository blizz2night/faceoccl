...
void detectAndDisplay(Mat face){
	...
	for (int i = 0; i < faces.size(); i++){
		...
		Mat frame;
		src.copyTo(frame);
		threshold(frame, frame, 0, 255, CV_THRESH_BINARY | CV_THRESH_OTSU);//二值化
		Mat frame_img(frame);
		Mat midImage;
		...
		//	Mat YUV[3];
		//	split(input, YUV);
		//	p2 = countNonZero(YUV[1] >= 133 & YUV[1] <= 173 & YUV[2] >= 77 & YUV[2] <= 127);
		//}
		//t2 = GetTickCount();
		//cout << t2 - t1 << endl;
		...
		//	split(input, YUV);
		//	Mat temp[3];
		//	threshold(YUV[1], temp[1], 133, 255, THRESH_TOZERO);
		//	threshold(YUV[1], temp[1], 173, 255, THRESH_TOZERO_INV);
		//	threshold(YUV[2], temp[2], 133, 255, THRESH_TOZERO);
		//	threshold(YUV[2], temp[2], 173, 255, THRESH_TOZERO_INV);
		//	//p3 = countNonZero(YUV[1].mul(YUV[2]));
		//}
		//t2 = GetTickCount();
		//cout << t2 - t1 << endl;
